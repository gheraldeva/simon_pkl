                ...
                <form action="" v-on:submit.prevent class="mt-5 pr-20 relative">
                    ...
                    <div class="my-3 flex">
                        ...
                        <input type="text" id="nis"
                            class="shadow-sm border bg-white text-gray-900 text-sm rounded-lg w-full p-2.5"
                            v-model=" siswaState.dataSiswa.nis  " />
                    </div>
...
export default {
        ...
        const Siswarules = computed(() => {
            return {
                dataSiswa: {
                    ...
                    nis: { required,numeric,maxLength: maxLength(9),minLength: minLength(9) },
    ...
    methods: {
        ...
        dataKelas(id) {
            axios.get(`http://localhost:2008/admin//findKelasFilter?id_jurusan=${id}`,{withCredentials:true})
