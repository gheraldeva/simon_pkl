...
export default {
    ...
    methods:{
        setFileType(){
            const file = this.laporan.file_laporan.split('.')
            this.filetipe = file[file.length - 1]
            const split = this.laporan.file_laporan.split('/')
        ...
        backToLaporan(){
            this.$router.push({path:"/laporandudi"})
        },
        downloadFile(){
            window.location.href = `http://localhost:2008/admin/downloadFileLaporan/${this.fileName}`
        },
        lihatFile(){
    ...
    beforeMount(){
        axios.get("http://localhost:2008/admin/findLaporanPkl/" + this.$route.params.id ,{withCredentials:true})
            .then((r) => {
